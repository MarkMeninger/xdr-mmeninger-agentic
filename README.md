# Cursor Project: Create Variable Advanced Search Query Language (ASQL) Requirements and related artifacts
...
Variable support in ASQL would allow new reports to be generated that could be re-used across all sensors saving hundreds of CSM hours and solve a business need asked by all iSensor customers.
...
2. The NIDS schema which would be the input source into the ASQL variables to solve the business problem as well as input into example queries that would be part of the MVP.
3. Taegis Advanced query user-docs to educate the LLM on ASQL syntax and functionality.
...
- **A) Taegis-Docs:** Source files from Taegis docs teams for several ASQL functionality areas used to train the LLM.
...
* ASQL-variable-support-requirements.txt -> Epic content
